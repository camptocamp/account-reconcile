...
class QoQaBaseExporter(Exporter):
...
class Translations(ConnectorUnit):
    ...
    def get_translations(self, record, normal_fields=None,
        ...
        * all the translations of the translatable attributes
        ...
        :param record: recordset of product or template
        ...
        lang_values = []
        for lang in langs:
            qoqa_lang_id = lang_binder.to_backend(lang)
            ...
            if attributes_unit is not None:
                attrs = self.unit_for(attributes_unit)
                values.update(attrs.get_values(record_in_lang,
                                               translatable=True))

            for src, target in normal_fields:
                values[target] = record_in_lang[src]

            values['locale'] = qoqa_lang_id
            lang_values.append(values)

        return {'translations_attributes': lang_values}
...
def export_record(session, model_name, binding_id, fields=None):
    ...
    with get_environment(session,
        ...
        exporter = connector_env.get_connector_unit(QoQaExporter)
